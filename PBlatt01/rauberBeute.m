...
%D = zeros(2,steps*steps);                %Vektor um alle Lösungen an Gitterpunkten fürs Vektorfeld zu speichern
...
switch choose
    ...
    for k=5:length(tau)
        ...
        %Matrix D befüllen, um Lösungen für Vektorfeld an den Gitterpunkten zu
        %erhalten
        for i=0:steps-1
           for j = 0:steps-1
    ...
    for k=1:length(tau)
        ...
        xlabel('Zeit')
        ylabel('Bestand')
        title("Räuber-Beute Zeitverlauf mit expl. RK3-Verfahren, Zeitschritt: " + tau(k) + " seconds")
        hold off
    end 
